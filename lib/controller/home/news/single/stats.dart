abstract class SingleNewsState{}

class InitSingleNewsStates extends SingleNewsState{}

// Send Like News StateSingleNewsState
class SendLikeNewsState extends SingleNewsState {}
class LoadingLikeNewsState extends SingleNewsState {}
class SuccessLikeNewsState extends SingleNewsState {}
class ErrorLikeNewsState extends SingleNewsState {}
// Get Statuses User
class GetUserLikeNewsState extends SingleNewsState {}
class LoadingGetUserLikeNewsState extends SingleNewsState {}
class SuccessGetUserLikeNewsState extends SingleNewsState {}
class ErrorGetUserLikeNewsState extends SingleNewsState {}
/// States For Comments
class SendCommentNewsState extends SingleNewsState {}
class LoadingCommentNewsState extends SingleNewsState {}
class SuccessCommentNewsState extends SingleNewsState {}
class ErrorCommentNewsState extends SingleNewsState {}
/// States For Drawer Bar
class LoadingSingleNewsDrawerState extends SingleNewsState {}
class SuccessSingleNewsDrawerState extends SingleNewsState {}
class ErrorSingleNewsDrawerState extends SingleNewsState {}
/// States For Recommended News
class LoadingRecommNewsState extends SingleNewsState {}
class SuccessRecommNewsState extends SingleNewsState {}
class ErrorRecommNewsState extends SingleNewsState {}
/// States For Update Comment News
class LoadingUpdateCommentNewsState extends SingleNewsState {}
class SuccessUpdateCommentNewsState extends SingleNewsState {}
class ErrorUpdateCommentNewsState extends SingleNewsState {}
/// States For Send Comment News
class LoadingSendCommentNewsState extends SingleNewsState {}
class SuccessSendCommentNewsState extends SingleNewsState {}
class ErrorSendCommentNewsState extends SingleNewsState {}
/// States For Get All Comment News
class LoadingGetCommentNewsState extends SingleNewsState {}
class SuccessGetCommentNewsState extends SingleNewsState {}
class ErrorGetCommentNewsState extends SingleNewsState {}
/// States For Get All Comment News
class LoadingDeleteCommentNewsState extends SingleNewsState {}
class SuccessDeleteCommentNewsState extends SingleNewsState {}
class ErrorDeleteCommentNewsState extends SingleNewsState {}
/// Send For View For View
class LoadingSendViewNewsState extends SingleNewsState {}
class SuccessSendViewNewsState extends SingleNewsState {}
class ErrorSendViewNewsState extends SingleNewsState {}
